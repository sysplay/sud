...
{
	FUNC_ENTER();
	//TODO 5.13: Check if (UART_LSR_DR) is set
...
static irqreturn_t serial_omap_irq(int irq, void *dev_id)
{
	...
	//TODO 6.4: Read IIR register and return if not a valid interrupt
	...
	// TODO 6.5: Invoke serial_omap_rdi, if UART_IIR_RDI is set and then return

	// TODO 6.6: Invoke serial_omap_rlsi, if UART_IIR_RLSI is set and then return
...
static int serial_omap_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		dev_err(&pdev->dev, "Unable to get the platform data\n");
		return -EINVAL;
	}
